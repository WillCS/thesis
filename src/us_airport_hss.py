from backbones import HighSalienceSkeletonBackboneStrategy
...
backbone_strategy = HighSalienceSkeletonBackboneStrategy()
visualisation     = MapVisualisation(data_provider, backbone_strategy)
plot_builder      = PlotBuilder(visualisation)

edges = [(v, u) for (v, u) in data_provider.graph.edges if data_provider.graph[v][u]["salience"] > 0.5]
